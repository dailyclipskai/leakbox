  ...
  return (
    <div className="space-y-3">
      <div className="glass p-2 flex items-center gap-2">
        ...
        <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search users..." className="flex-1 bg-transparent outline-none py-2 px-1 text-sm placeholder:text-muted-foreground" />
      </div>
      {filtered === null ? (
            ...
            return (
              <Link key={u.id} to="/u/$username" params={{ username: u.username }} className="glass neon-hover flex items-center gap-3 p-3">
                ...
                <div className="text-right text-xs text-muted-foreground">
                  <div><span className="text-foreground font-semibold">{c.posted}</span> posted</div>
                  <div><span className="text-foreground font-semibold">{c.verified}</span> verified</div>
                </div>
              </Link>
            );
          })}
...
export function Avatar({ p, size = 40 }: { p: { username: string; profile_picture: string | null }; size?: number }) {
  return (
    <div className="rounded-full bg-muted border border-border flex items-center justify-center overflow-hidden shrink-0" style={{ width: size, height: size }}>
      {p.profile_picture ? <img src={p.profile_picture} alt="" className="w-full h-full object-cover" /> : <span className="text-sm">{p.username[0]?.toUpperCase()}</span>}
    </div>
  );
}
